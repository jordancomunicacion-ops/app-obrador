...
title SOTOdelPRIOR - SETUP UNICO del VPS de Cocina (modelo git-pull)
cd /d "%~dp0"
...
set REPO_NAME=app-cocina

cls
echo ============================================================
echo   SETUP UNICO - VPS COCINA (modelo git-pull)
...
ssh %REMOTE_USER%@%REMOTE_HOST% "set -e; cd %REMOTE_PARENT%; if [ -d cocina ]; then cp cocina/.env /root/cocina.env.bak 2>/dev/null || cp cocina/.env.production /root/cocina.env.bak 2>/dev/null || echo 'AVISO: no se encontro .env, lo restauraras manualmente'; mv cocina cocina_old_$(date +%%Y%%m%%d-%%H%%M%%S); echo 'Carpeta anterior renombrada.'; else echo 'No habia carpeta cocina previa.'; fi"
if errorlevel 1 goto error

echo.
echo [2/5] Clonando el repo (privado) con tu token...
ssh %REMOTE_USER%@%REMOTE_HOST% "set -e; cd %REMOTE_PARENT%; git clone https://%GHUSER%:%GHTOKEN%@github.com/%REPO_OWNER%/%REPO_NAME%.git cocina; cd cocina; git remote set-url origin https://github.com/%REPO_OWNER%/%REPO_NAME%.git; git config credential.helper store; printf 'https://%GHUSER%:%GHTOKEN%@github.com\n' > ~/.git-credentials; chmod 600 ~/.git-credentials; echo 'Repo clonado y credenciales guardadas para futuros pull.'"
if errorlevel 1 goto error

echo.
echo [3/5] Restaurando .env de produccion...
ssh %REMOTE_USER%@%REMOTE_HOST% "if [ -f /root/cocina.env.bak ]; then cp /root/cocina.env.bak %REMOTE_PATH%/.env; echo '.env restaurado.'; else echo 'AVISO: no hay backup de .env; copialo manualmente a %REMOTE_PATH%/.env antes de continuar.'; fi"
if errorlevel 1 goto error

echo.
echo [4/5] Arranque inicial (build + db push)...
ssh %REMOTE_USER%@%REMOTE_HOST% "set -e; cd %REMOTE_PATH%; export AUTH_URL=https://cocina.sotodelprior.com; docker compose -f docker-compose.yml up -d --build --remove-orphans; sleep 5; docker compose -f docker-compose.yml exec -T sotococina-web npx prisma@5.22.0 db push --accept-data-loss --skip-generate"
if errorlevel 1 goto error
...
echo     ssh %REMOTE_USER%@%REMOTE_HOST% "rm -rf %REMOTE_PARENT%/cocina_old_*"
echo ============================================================
pause
endlocal
exit /b 0
...
echo   %REMOTE_PARENT%/cocina_old_*  y la BD esta intacta.
echo   Para reintentar, primero entra por SSH y elimina la
echo   carpeta cocina a medias:  rm -rf %REMOTE_PATH%
echo ============================================================
pause
endlocal
exit /b 1
