# Setup único del despliegue de obrador (modelo git-pull, igual que contabilidad)

> El `DESPLEGAR.bat` funciona como el de contabilidad: **el servidor tiene el repo
...
## Paso único — forma fácil: `SETUP-VPS-OBRADOR.bat`
...
1. Doble clic en **`SETUP-VPS-OBRADOR.bat`** (está en la raíz del repo, junto a `DESPLEGAR.bat`).
...
`/root/SOTOdelPRIOR/apps/obrador_old_*` y la base de datos está intacta.

---

## Paso único — forma manual (alternativa, si prefieres comando a comando)

La carpeta del servidor `/root/SOTOdelPRIOR/apps/obrador` debe ser un clon del repo.
Hay que conservar los volúmenes de base de datos (que son externos a la carpeta, así que no se tocan).
...
2. **Respaldo de seguridad de la carpeta actual** (por si acaso):
   ...
   cp obrador/.env /root/obrador.env.bak    # guarda el .env de producción
   mv obrador obrador_old_$(date +%Y%m%d)
   ```
   > La base de datos NO está en esta carpeta (usa el volumen Docker externo `cocina_cocina_db_data`,
   > que se conserva con ese nombre físico), así que renombrar la carpeta no afecta a los datos.

3. **Clona el repo** en la ruta esperada:
   ```bash
   cd /root/SOTOdelPRIOR/apps
   git clone https://github.com/jordancomunicacion-ops/app-obrador.git obrador
   cd obrador
   ```
...
6. Comprueba que responde:
   ...
   `rm -rf /root/SOTOdelPRIOR/apps/obrador_old_*`
...
```bash
ssh-keygen -t ed25519 -f ~/.ssh/obrador_deploy -N "" -C "obrador-deploy"
cat ~/.ssh/obrador_deploy.pub
```
Añade esa clave pública en GitHub → repo `app-obrador` → Settings → Deploy keys (solo lectura).
Luego clona con: `GIT_SSH_COMMAND='ssh -i ~/.ssh/obrador_deploy -o IdentitiesOnly=yes' git clone git@github.com:jordancomunicacion-ops/app-obrador.git obrador`
...
| 9. Backup BD | `pg_dump` comprimido en `/backups/obrador/`. |
| 10. SSH al VPS | Abre una sesión SSH al servidor. |

> ⚠️ **Deuda pendiente (igual que en contabilidad):** obrador usa `prisma db push --accept-data-loss`
