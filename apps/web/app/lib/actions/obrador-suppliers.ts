...
export type ObradorSupplierFormState = {
  errors?: { name?: string[] };
  message: string | null;
};
...
export async function createObradorSupplier(
  ...
  revalidatePath('/dashboard/obrador/suppliers');
  redirect('/dashboard/obrador/suppliers');
}

export async function updateObradorSupplier(
  id: string,
  prevState: ObradorSupplierFormState,
  formData: FormData,
): Promise<ObradorSupplierFormState> {
...
export async function deleteObradorSupplier(id: string) {
  const existing = await prisma.supplier.findFirst({
    where: { id, ...(await locationScope()) },
    select: { id: true },
  });
